...
from davit.utils.postmortem_query_thread import PostMortemQueryThread
from davit.utils.postmortem_threads_panel_table_model import PostMortemThreadsPanelTableModel
...
class PostMortemThreadsPanel(QDialog):
    ...
    def __init__(self, app, app_root_path = None, window_icon_path = None, parent = None, qta_str_icon = "mdi.spider-thread"):
        ...
        self.row_height = 25
        self.max_threads = 25
        self.column_names = ["System", "Class Name", "Source", "TS1", "TS2", "Elapsed Time"]
        ...
        # set title and icon
        self.setWindowTitle("PostMortem Query Threads Panel")
        ...
        # apply style
        with open(os.path.join(self.app_root_path, "resources", "qss", "postmortem_threads_panel.qss"), "r") as file_qss:
    ...
    def buildCodeWidgets(self):
        ...
        self.label_info.setText("In this panel you can find all the PostMortem running threads for the introduced queries. Note that you can stop/quit queries by right-clicking on the table.")
    ...
    def initTable(self):
        ...
        # set model
        self.table_model = PostMortemThreadsPanelTableModel(data = data, header_labels = self.column_names)
    ...
    def addThread(self, system, class_name, source, ts1, ts2):
        ...
        thread = PostMortemQueryThread(system, class_name, source, ts1, ts2, self.id_counter)

        # add thread to the list
        self.query_threads.append(thread)

        # update the data
        self.data.append([system, class_name, source, ts1, ts2, "00:00:00"])
        ...
        if True:
            ...
            query = "{}/{}/{}".format(system, class_name, source)

            # transform the timestamps
            ts1 = datetime.fromtimestamp(ts1 / 1e9).strftime("%Y-%m-%d %H:%M:%S.%f")[:-3]
            ts2 = datetime.fromtimestamp(ts2 / 1e9).strftime("%Y-%m-%d %H:%M:%S.%f")[:-3]
    ...
    def handleQueryResult(self, result, verbose = False):
        ...
        response_df, system, class_name, source, ts1, ts2, id, error = result

        # for debugging
        if verbose:
            print("SYSTEM = {}, CLASS_NAME = {}, SOURCE = {}, TS1 = {}, TS2 = {}".format(system, class_name, source, ts1, ts2))
            print(response_df)
        ...
        if True:
            ...
            if id in self.ids:
                ...
                self.parent.refreshTreeView(self.tree_items[self.ids.index(id)], query, response_df, error)
