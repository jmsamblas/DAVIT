...
class PostMortemTreeViewModel(QStandardItemModel):
    ...
    def add_results_to_query(self, root_query, query, response_df, error):
        ...
        if not response_df.empty:
            ...
            grouped_df = response_df.groupby(['header.systemName', 'header.className', 'header.sourceName'])

            # iterate over groups
            for group_name, group_df in grouped_df:

                # sort based on acquisition times
                sorted_group_df = group_df.sort_values(by='values.acqStamp.value', ascending=True)

                # add noeds
                self.build_key_tree(group_name, root_query, sorted_group_df)

        return

    #----------------------------------------------#

    def build_key_tree(self, group_name, root, sorted_group_df):

        # add the node for that key
        node = self.add_node(root, node_name = "{}/{}/{}".format(group_name[0], group_name[1], group_name[2]), node_data = np.array([]), qta_icon_str = "mdi.focus-field")

        # get data columns
        data_columns = [col for col in sorted_group_df.columns if re.compile(r'^values.*\.value$').match(col)]

        # iterate over columns
        for column_name in data_columns:

            # get formatted name
            formatted_name = column_name.split(".")[1]

            # create new node for that field
            field_node = self.add_node(node, node_name=formatted_name, node_data = np.array([]), qta_icon_str = "mdi.focus-field")

            # now iterate over timestamps
            for row in range(0, sorted_group_df.shape[0]):

                # get acquisition time
                acq_stamp = sorted_group_df.iloc[row]['values.acqStamp.value'] / 1E9

                # get ts in readable way
                formatted_ts = datetime.fromtimestamp(acq_stamp).strftime('%Y-%m-%d %H:%M:%S.%f')

                # get node data
                node_data = sorted_group_df.iloc[row][column_name]

                # convert to array (e.g. scalars or lists)
                if not isinstance(node_data, np.ndarray):
                ...
                # transpose it
                if node_data.ndim > 1:
                    if node_data.shape[1] > node_data.shape[0]:
                        node_data = np.transpose(node_data)

                # add the data node
                self.add_node(field_node, node_name=formatted_ts, node_data=node_data, qta_icon_str="mdi.data-matrix")
